...
# Due Date: 2/13/2022
# Description: Implement a class that utilizes a circular buffer
...
class Queue:
    ...
    def enqueue(self, value: object) -> None:
        """
        Adds a new value at the end of the queue based on the given value.
        """
        if self._current_size == self._sa.length():
            self._double_queue()    #Doubles size of queue if maximum is reached.
    ...
    def dequeue(self) -> object:
        """
        Removes the value at the beginning of the queue and then returns it.
        Returns Exception if no more values are in queue.
        """
    ...
    def front(self) -> object:
        """
        Use this method to determine the value that comes first in the queue without editing.
    ...
    def _double_queue(self) -> None:
        """
        Double the size of the Static Array.
        """
        new_size = self._sa.length() * 2
        new_sa = StaticArray(new_size)
        i = 0
        for j in range(self._front, self._front + self._current_size):# copy elements from front to end
            new_sa[i] = self._sa[j % self._sa.length()]
            i += 1
